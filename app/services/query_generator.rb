...
class QueryGenerator
  ...
  SOURCE_RULES = {
    ...
    "threads" => <<~RULE
      # threads の書き方
      Threadsの keyword_search は投稿の本文をキーワードで探す。日本語の利用者が多く、
      事業者の発信もあるので、Blueskyより言葉を選べる。

      - 1〜2語を中心にする（3語以上は当たりにくい）
      - 半分は「主題語＋買う側の短い動詞・形容詞」（例: ホームページ 作りたい／見積もり 高い）
      - 残りは主題語1語（例: ホームページ制作／ネットショップ）。ただし広すぎる1語（サイト・ツール）は避ける
      - 長い言い回し・助詞・疑問形は入れない

      良い例: ホームページ 作りたい ／ 制作会社 選び方 ／ ネットショップ 売れない ／ 業務効率化 したい
      悪い例: サイト（広すぎる）／ ホームページ どこに頼めばいい（長すぎる）
    RULE
  }.freeze
